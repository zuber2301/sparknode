...
-- NOTE: No user cleanup here. The 4 seed accounts above use ON CONFLICT DO UPDATE
-- so they are always restored to known-good state. All other users created through
-- the platform UI (tenant managers, employees, etc.) are intentionally preserved.
-- Only orphaned system_admin rows (no matching user) are cleaned up.
DELETE FROM system_admins WHERE user_id NOT IN (SELECT id FROM users);
